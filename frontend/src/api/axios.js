...
apiClient.interceptors.request.use(
  (config) => {
    ...
    const isTokenFreeEndpoint = tokenFreeEndpoints.some((endpoint) =>
    ...
    if (!isTokenFreeEndpoint) {
      ...
      if (token) {
        ...
        if (userData) {
          console.error(
            "❌ User data exists but token missing! This is a bug."
          );
          ...
          if (data.message) {
            ...
            if (data.data && typeof data.data === "object") {
              const errorMessages = Object.entries(data.data)
                .map(
                  ([field, message]) => `<strong>${field}:</strong> ${message}`
                )
                .join("<br>");
