...
const OrganizationReviewModal = ({
      ...
      try {
        ...
        if (documentPath.includes("blob.core.windows.net")) {
          ...
          const response = await fetch(
            `${API_BASE_URL}/files/business-registration/${filename}`,
            {
              headers: {
                Authorization: `Bearer ${token}`
              }
  ...
  const handleApprove = useCallback(async () => {
    ...
    try {
      await dispatch(
        approveOrganization(organization.organizationUuid)
      ).unwrap();
  ...
  const handleReject = useCallback(async () => {
    ...
    try {
      await dispatch(
        rejectOrganization(organization.organizationUuid)
      ).unwrap();
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 mt-3">
                  <div
                    className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-sm font-medium border ${status.color}`}
                  >
...
const InfoCard = ({
  ...
  <div
    className={`bg-white border border-gray-200 rounded-lg p-4 ${fullWidth ? "md:col-span-2" : ""}`}
  >
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2.5">
        <Icon
          className={`w-4 h-4 ${module.enabled ? module.color : "text-gray-400"}`}
        />
        <span
