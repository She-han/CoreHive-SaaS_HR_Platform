      ...
      <div className="flex items-start space-x-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center justify-between mb-1">
            ...
            <span
              className={`${priority.bgColor} ${priority.textColor} text-xs px-2 py-0.5 rounded-full font-medium`}
            >
              {insight.priority}
            </span>
          </div>
          ...
          {isExpanded && insight.action && (
            <div className="mt-3 pt-3 border-t border-current border-opacity-20">
              ...
              <p className={`text-sm ${config.textColor}`}>{insight.action}</p>
            </div>
          )}
          ...
          {insight.action && (
            <div className="mt-2 flex items-center text-xs opacity-60">
              <span>
                {isExpanded ? "Click to collapse" : "Click for action"}
              </span>
              <svg
...
const AIInsightsCard = ({ organizationUuid }) => {
  ...
  const fetchInsights = useCallback(async () => {
    ...
    try {
      const response =
        await aiInsightsApi.getDashboardInsights(organizationUuid);
          ...
          <div className="flex items-center justify-between text-xs text-gray-500">
            <div className="flex items-center space-x-4">
              {metrics.employee_stats && (
                <span>
                  👥 {metrics.employee_stats.active_employees || 0} active
                  employees
                </span>
              )}
              {metrics.attendance_analysis && (
                <span>
                  📊 {metrics.attendance_analysis.attendance_rate || 0}%
                  attendance
                </span>
              )}
              {metrics.leave_analysis && (
                <span>
                  📋 {metrics.leave_analysis.status_breakdown?.PENDING || 0}{" "}
                  pending leaves
                </span>
              )}
            </div>
