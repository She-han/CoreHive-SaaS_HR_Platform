        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            <h1 className="text-3xl font-extrabold text-[#0C397A] tracking-tight md:text-4xl">
              Dashboard <span className="text-[#9B9B9B] font-light">|</span>{" "}
              <span className="text-[#02C39A]">CoreHive</span>
            </h1>
        ...
        <div className="lg:col-span-8 bg-white p-8 m-6 rounded-3xl shadow-[0_10px_40px_rgba(0,0,0,0.03)] border border-white/50">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
            {[
              ...
              <button
                key={item.label}
                className="flex flex-col items-start p-5 rounded-2xl bg-[#F1FDF9]/30 border border-transparent hover:border-[#02C39A] hover:bg-white hover:shadow-lg transition-all group text-left"
              >
                <span className="text-[9px] font-black uppercase text-[#9B9B9B] group-hover:text-[#02C39A] mb-1">
                  {item.label}
                </span>
                <span className="text-xl font-bold text-[#0C397A]">
                  {item.count}
                </span>
                <span className="text-[10px] text-[#9B9B9B] mt-1 italic">
                  {item.sub}
                </span>
              </button>
            ))}
          ...
          <div className="space-y-8 relative before:absolute before:inset-0 before:left-[19px] before:w-px before:bg-gray-100">
            {[
              ...
              <div key={idx} className="flex gap-5 items-start relative z-10">
                <div
                  className="p-2 rounded-full bg-white border border-gray-100 text-gray-400 shadow-sm"
                  style={{ color: activity.color }}
                >
                  {activity.icon}
                </div>
                <div>
                  <p className="text-sm font-bold text-[#333333] leading-none">
                    {activity.user}
                  </p>
                  <p className="text-xs text-[#9B9B9B] mt-1">
                    {activity.action}
                  </p>
                  <span className="inline-block text-[9px] font-bold text-[#02C39A] bg-[#F1FDF9] px-2 py-0.5 rounded mt-2">
                    {activity.time}
                  </span>
                </div>
              </div>
