    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="bg-white p-5 rounded-3xl shadow-sm border border-[#F1FDF9] flex flex-col md:flex-row justify-between items-center gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h3 className="text-[#0C397A] font-bold text-lg">
              Annual Growth Analytics
            </h3>
            <p className="text-[#9B9B9B] text-xs font-medium uppercase tracking-widest">
              Yearly hiring velocity
            </p>
          </div>
        </div>
      ...
      {data ? (
              ...
              {Object.entries(data).map(([month, count]) => (
                ...
                >
                  ...
                  <div className="flex items-end justify-between">
                    <p className="text-2xl font-black text-[#0C397A]">
                      {count}
                    </p>
                    {count > 0 && (
                      <ArrowUpRight size={16} className="text-[#1ED292] mb-1" />
                    )}
                  </div>
                  {/* Subtle bar visual */}
                  <div
                    className="absolute bottom-0 left-0 h-1 bg-[#1ED292] rounded-full transition-all duration-500 opacity-0 group-hover:opacity-100"
                    style={{ width: count > 0 ? "40%" : "0%" }}
                  ></div>
                </div>
              ))}
            </div>
          ...
          <div className="xl:col-span-1 space-y-6">
            <div className="bg-[#0C397A] p-8 rounded-[2rem] text-white h-full flex flex-col justify-between relative overflow-hidden shadow-xl shadow-[#0C397A]/20">
              <div className="relative z-10">
                <p className="text-blue-200/60 text-xs font-bold uppercase tracking-widest mb-2">
                  Annual Total
                </p>
                ...
                <div className="space-y-4 mt-8">
                  <div className="bg-white/10 p-4 rounded-2xl backdrop-blur-md">
                    <p className="text-blue-100/60 text-[10px] font-bold uppercase">
                      Peak Month
                    </p>
                    <p className="text-lg font-bold">
                      {getMonthName(
                        Object.entries(data).sort((a, b) => b[1] - a[1])[0][0]
                      )}
                    </p>
                  </div>
                  <p className="text-sm text-blue-100/80 leading-relaxed">
                    Hiring trends for {year} show a
                    <span className="text-[#02C39A] font-bold ml-1">
                      Positive
                    </span>{" "}
                    growth trajectory.
                  </p>
        ...
        <div className="bg-white rounded-[2rem] p-20 border border-dashed border-[#9B9B9B]/30 flex flex-col items-center justify-center text-center">
          ...
          <p className="text-[#9B9B9B] text-sm max-w-xs mt-2">
            Select a fiscal year to visualize the organization's hiring velocity
            and monthly trends.
          </p>
        </div>
      )}
