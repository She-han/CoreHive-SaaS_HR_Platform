  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px]">
        ...
        <p className="mt-4 text-[#9B9B9B] font-medium animate-pulse">
          Loading CoreHive Analytics...
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 p-4 md:p-8 bg-[#F1FDF9]/30 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row justify-between items-end gap-4">
        <div>
          <h2 className="text-4xl font-black text-[#0C397A] tracking-tight">
            Headcount
          </h2>
          <p className="text-[#9B9B9B] font-medium">
            Detailed workforce distribution across the organization.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <section className="space-y-4">
          <div className="flex items-center gap-2 px-2">
            ...
            <h3 className="text-[#0C397A] font-bold text-lg uppercase tracking-wide">
              By Department
            </h3>
          </div>
          <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
            <table className="w-full">
              <thead className="bg-[#F1FDF9]/50 border-b border-gray-100">
                <tr>
                  <th className="px-6 py-4 text-left text-[#05668D] text-xs font-bold uppercase">
                    Department
                  </th>
                  <th className="px-6 py-4 text-right text-[#05668D] text-xs font-bold uppercase">
                    Count
                  </th>
                </tr>
              </thead>
        ...
        <section className="space-y-4">
          ...
          <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
            <table className="w-full">
              <thead className="bg-[#F1FDF9]/50 border-b border-gray-100">
                <tr>
                  <th className="px-6 py-4 text-left text-[#1ED292] text-xs font-bold uppercase">
                    Job Title / Role
                  </th>
                  <th className="px-6 py-4 text-right text-[#1ED292] text-xs font-bold uppercase">
                    Count
                  </th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-50">
                {data.byDesignation.map(([name, count]) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-right font-bold text-[#0C397A]">
                      {count}
                    </td>
                  </tr>
                ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="md:col-span-2 bg-[#0C397A] p-8 rounded-3xl text-white flex flex-col md:flex-row items-center justify-between gap-6 relative overflow-hidden">
          <div className="relative z-10">
            ...
            <p className="text-blue-100/70 max-w-md text-sm">
              Your organization currently operates with{" "}
              <strong>{data.byDepartment.length}</strong> departments and
              <strong> {data.byDesignation.length}</strong> unique roles. The
              largest group is <strong>{data.byDepartment[0]?.[0]}</strong>.
            </p>
          </div>
          <button
        ...
        <div className="bg-white p-8 rounded-3xl border border-[#F1FDF9] flex flex-col justify-center items-center text-center">
          ...
          <p className="text-[#9B9B9B] text-xs font-bold uppercase">
            Efficiency Index
          </p>
