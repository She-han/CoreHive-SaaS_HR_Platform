...
const RequestSupportOrBugReport = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Check monthly ticket limit (3 tickets per month)
    const now = new Date();
    const currentMonth = now.getMonth();
    const currentYear = now.getFullYear();
    
    const ticketsThisMonth = myTickets.filter(ticket => {
      const ticketDate = new Date(ticket.createdAt);
      return ticketDate.getMonth() === currentMonth && 
             ticketDate.getFullYear() === currentYear;
    });

    if (ticketsThisMonth.length >= 3) {
      Swal.fire({
        icon: "error",
        title: "Monthly Limit Reached",
        html: `You have reached the maximum limit of <strong>3 support tickets</strong> for this month.<br><br>
               <span style="font-size: 14px; color: #6B7280;">Tickets submitted this month: ${ticketsThisMonth.length}/3</span><br>
               <span style="font-size: 14px; color: #6B7280;">Please wait until next month to submit more tickets.</span>`,
